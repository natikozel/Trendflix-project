...
describe('User Input Preferences', () => {
  ...
  test('User can submit representative input "A romantic comedy with a happy ending" and receive appropriate recommendations', async () => {
    // Mock successful response with romantic comedy recommendations
    (global.fetch as jest.Mock).mockImplementationOnce(() => 
      Promise.resolve({
        ok: true,
        json: () => Promise.resolve([
          { 
            ...
            metadata: { 
              posterUrl: '/eternal-sunshine.jpg',
    ...
    // Fill out the form with the representative input
    const textInput = screen.getByLabelText(/Tell us what kind of movies you like/i);
    fireEvent.change(textInput, { target: { value: 'A romantic comedy with a happy ending' } });
    ...
    expect(requestBody.freeText).toBe('A romantic comedy with a happy ending');
    
    // Verify that the response contains appropriate romantic comedy recommendations
    const state = mockStore.getState();
    expect(state.recommendations.items).toBeDefined();
  });

  test('User input preferences support both text and structured format', async () => {
    // Mock successful response
    (global.fetch as jest.Mock).mockImplementationOnce(() => 
      Promise.resolve({
        ok: true,
        json: () => Promise.resolve([
          { movieId: '456', movieName: 'Structured Input Movie', metadata: { posterUrl: '/structured.jpg' } }
        ]),
      }) as Promise<Response>
    );
    ...
    // Test structured input format (if form supports it)
    const textInput = screen.getByLabelText(/Tell us what kind of movies you like/i);
    fireEvent.change(textInput, { 
      target: { 
        value: 'Genre: Action, Year: 2020-2023, Rating: PG-13, Duration: 90-120 minutes' 
      } 
    });
    ...
    // Verify the structured input was processed
    expect(fetch).toHaveBeenCalledTimes(1);
    const fetchCall = (global.fetch as jest.Mock).mock.calls[0];
    const requestOptions = fetchCall[1];
    const requestBody = JSON.parse(requestOptions.body);
    
    expect(requestBody.freeText).toBe('Genre: Action, Year: 2020-2023, Rating: PG-13, Duration: 90-120 minutes');
  });
}); 